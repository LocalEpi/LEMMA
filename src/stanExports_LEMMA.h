...
public:
    ...
    void ctor_body(stan::io::var_context& context__,
        ...
        try {
            ...
            context__.validate_dims("data initialization", "panic_level", "double", context__.to_vec());
            panic_level = double(0);
            vals_r__ = context__.vals_r("panic_level");
            pos__ = 0;
            panic_level = vals_r__[pos__++];
            check_greater_or_equal(function__, "panic_level", panic_level, 0.0);
    ...
    T__ log_prob(std::vector<T__>& params_r__,
        ...
        try {
            ...
            int panic(0);
            (void) panic;  // dummy to suppress unused var warning
            stan::math::fill(panic, std::numeric_limits<int>::min());
            ...
            stan::math::assign(panic, 0);
            current_statement_begin__ = 166;
            for (int it = 1; it <= (nt - 1); ++it) {
                current_statement_begin__ = 170;
                if (as_bool((primitive_value(logical_eq(panic, 0)) && primitive_value(logical_gt((get_base1(x, Hmod, it, "x", 1) + get_base1(x, Hicu, it, "x", 1)), panic_level))))) {
                    current_statement_begin__ = 171;
                    stan::math::assign(panic, 1);
                    current_statement_begin__ = 172;
                    for (int jt = 1; jt <= nt; ++jt) {
                        current_statement_begin__ = 173;
                        stan::model::assign(beta, 
                                    stan::model::cons_list(stan::model::index_uni(jt), stan::model::nil_index_list()), 
                                    (get_base1(beta, jt, "beta", 1) * pow(0.5, inv_logit(((9.19024 / 7.0) * (jt - (it + (7.0 / 2))))))), 
                                    "assigning variable beta");
                    }
                }
