...
parameters {
  ...
  real<offset=mu_duration_latent, multiplier=sigma_duration_latent> duration_latent; // duration is a minimum of 1 which is the stepsize of this model
  real<offset=mu_duration_rec_mild, multiplier=sigma_duration_rec_mild> duration_rec_mild;
  real<offset=mu_duration_pre_hosp, multiplier=sigma_duration_pre_hosp> duration_pre_hosp;
  real<offset=mu_duration_hosp_mod, multiplier=sigma_duration_hosp_mod> duration_hosp_mod;
  real<offset=mu_duration_hosp_icu, multiplier=sigma_duration_hosp_icu> duration_hosp_icu;
  //
  real<offset=mu_frac_hosp, multiplier=sigma_frac_hosp> frac_hosp;
  real<offset=mu_frac_icu, multiplier=sigma_frac_icu> frac_icu;
  real<offset=mu_frac_mort, multiplier=sigma_frac_mort> frac_mort;
  real<offset=mu_frac_tested, multiplier=sigma_frac_tested> frac_tested;

  // real<offset=mu_iniE, multiplier=sigma_iniE> ini_E; //this one is causing problems - because exponential
  ...
  //
  real<offset=mu_r0, multiplier=sigma_r0> r0;
  real<offset=mu_beta_inter, multiplier = sigma_beta_inter> beta_multiplier[ninter];
  real<offset=mu_t_inter, multiplier = sigma_t_inter> t_inter[ninter];
...
transformed parameters {
  ...
  real<lower=1.0> duration_latent_copy;
  real<lower=1.0> duration_rec_mild_copy;
  real<lower=1.0> duration_pre_hosp_copy;
  real<lower=1.0> duration_hosp_mod_copy;
  real<lower=1.0> duration_hosp_icu_copy;

  // real<lower=0.005, upper=1.0> frac_hosp_copy;
  real<lower=0.0, upper=1.0> frac_hosp_copy;
  real<lower=0.0, upper=1.0> frac_icu_copy;
  real<lower=0.0, upper=1.0> frac_mort_copy;
  real<lower=0.0, upper=1.0> frac_tested_copy;

  real<lower=0.0> ini_E_copy;
  // real<lower=0.0> ini_Imild[1 - from_beginning];
  // real<lower=0.0> ini_Ipreh[1 - from_beginning];
  // real<lower=0.0> ini_Rlive[1 - from_beginning];
  real<lower=0.0> sigma_obs_copy[nobs_types];


  real<lower=0.0> r0_copy;
  real<lower=0.0> beta_multiplier_copy[ninter];
  real<lower=1.0> t_inter_copy[ninter];

  row_vector<multiplier = sigma_obs[obs_hosp_census]>[nobs[obs_hosp_census]] error_hosp;
  row_vector<multiplier = sigma_obs[obs_icu_census]>[nobs[obs_icu_census]] error_icu;
  row_vector<multiplier = sigma_obs[obs_cum_deaths]>[nobs[obs_cum_deaths]] error_mort;
  row_vector<multiplier = sigma_obs[obs_admits]>[nobs[obs_admits]] error_admits;
  row_vector<multiplier = sigma_obs[obs_cases]>[nobs[obs_cases]] error_cases;
  row_vector<multiplier = sigma_obs[obs_seroprev]>[nobs[obs_seroprev]] error_seroprev;
...
duration_latent_copy = duration_latent;
duration_rec_mild_copy = duration_rec_mild;
duration_pre_hosp_copy = duration_pre_hosp;
duration_hosp_mod_copy = duration_hosp_mod;
duration_hosp_icu_copy = duration_hosp_icu;

frac_hosp_copy = frac_hosp;
frac_icu_copy = frac_icu;
frac_mort_copy = frac_mort;
frac_tested_copy = frac_tested;

ini_E_copy = ini_E;
sigma_obs_copy = sigma_obs;

r0_copy = r0;
beta_multiplier_copy = beta_multiplier;
t_inter_copy = t_inter;
  ...
  for (itype in 1:nobs_types) {
    if (itype == obs_hosp_census) {
      ...
      error_hosp = sim_data[itype, tobs[itype, 1:nobs[itype]]] - obs_data[itype, 1:nobs[itype]];
    } else if (itype == obs_icu_census) {
      sim_data[itype] = x[Hicuu] + x[Hicuv];
      error_icu = sim_data[itype, tobs[itype, 1:nobs[itype]]] - obs_data[itype, 1:nobs[itype]];
    } else if (itype == obs_cum_deaths) {
      sim_data[itype] = x[Rmort];
      error_mort = sim_data[itype, tobs[itype, 1:nobs[itype]]] - obs_data[itype, 1:nobs[itype]];
    } else if (itype == obs_admits) {
      sim_data[itype] = new_admits;
      error_admits = sim_data[itype, tobs[itype, 1:nobs[itype]]] - obs_data[itype, 1:nobs[itype]];
    } else if (itype == obs_cases) {
      sim_data[itype] = new_cases;
      error_cases = sim_data[itype, tobs[itype, 1:nobs[itype]]] - obs_data[itype, 1:nobs[itype]];
    } else if (itype == obs_seroprev) {
      sim_data[itype] = (x[Sv] + x[Ev] + x[Imildv] + x[Iprehv] + x[Hmodv] + x[Hicuv] + x[Rliveu] + x[Rlivev]) / npop;
      error_seroprev = sim_data[itype, tobs[itype, 1:nobs[itype]]] - obs_data[itype, 1:nobs[itype]];
    } else {
...
model {
  if (extend == 0) {
    ...
    //   }
    // }
    error_hosp ~ normal(0, sigma_obs[obs_hosp_census]);
    error_icu ~ normal(0, sigma_obs[obs_icu_census]);
    error_mort ~ normal(0, sigma_obs[obs_cum_deaths]);
    error_admits ~ normal(0, sigma_obs[obs_admits]);
    error_cases ~ normal(0, sigma_obs[obs_cases]);
    error_seroprev ~ normal(0, sigma_obs[obs_seroprev]);
  }
}
