...
hosp.bounds <- dt[, .(LowerBound = sum(PatientCount * mult.low), UpperBound = sum(PatientCount * mult.high)), by=date]
hosp.bounds
```
...
```{r}
hosp.bounds <- hosp.bounds[!date %in% as.Date(c("2020-03-30", "2020-03-31", "2020-04-03", "2020-04-04"))]
```
...
Copy and paste the bounds to the Hospitilization Data tab.
```{r}
write.table(hosp.bounds[, .(date, LowerBound, UpperBound)], sep = ",", row.names = F, col.names = F)
```
...
Now we at least have 117 iterations.

I continued this process and eventually was able to tighten lower.bound.multiplier to 0.98 and upper.bound.multiplier to	1.02. With main.iterations = 200000, I came up with niter = 825.
