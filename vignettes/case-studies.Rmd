...
1. Get raw data. 
It would be better to automate this, but to keep things simple in this example, we exported as csv from https://data.sfgov.org/COVID-19/COVID-19-Hospitalizations/nxjg-bhem. This is COVID-19_Hospitalizations_SFMay4.csv.
...
```
Note: You will get the same .pdf and .xlsx output files without the `result <-`
```{r, eval = F}
LEMMA::CredibilityIntervalFromExcel("SF-May4.xlsx")  
```
Adding `result <-` gives you more flexibility to use the outputs of LEMMA within R.

```{r, echo=FALSE}
CachePlot("SF-May4.rds", print.long.term = F)
```
...
```{r, echo=FALSE}
CachePlot("SF-May4.rds", print.short.term = F)
```
...
```{r, echo = F}
 if (eval.all) {
    gplot.hist <- PlotHist(result$inputs$all.params$hosp.length.of.stay, GetPlotTitle(sum(result$in.bounds)), "", "hosp.length.of.stay", result$in.bounds)
